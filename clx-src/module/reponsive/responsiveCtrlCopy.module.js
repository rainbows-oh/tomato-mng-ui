...
cpr.controls.Control.prototype._removeEventListener = cpr.controls.Control.prototype.removeEventListener;
cpr.controls.Control.prototype.removeEventListener = function(type, listener) {
	
	this._removeEventListener(type, listener);

	if (this._eventListenerList) {
		var eventArray = this._eventListenerList[type];
		if(eventArray) {
			var vnEvnetIdx = eventArray.indexOf(listener);
			if(vnEvnetIdx > -1) {
				eventArray.splice(vnEvnetIdx, 1);
			}
		}
	}
}
