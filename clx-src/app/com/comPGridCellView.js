...
function createFormContent() {
	...
	if(ValueUtil.fixBoolean(voInitValue["editable"])) {
		app.lookup("btnDelete").visible = true;
		app.lookup("btnSave").visible = true;
	}
	
	var RGrid = vcGrid["_RGrid"];
	...
	(function(container) {
		var vcGrpForm = new cpr.controls.Container("grpForm");
		vcGrpForm.style.setClasses(["form-base"]);
		var voFormLot = new cpr.controls.layouts.FormLayout();
		voFormLot.scrollable = false;
		voFormLot.topMargin = "6px";
		voFormLot.rightMargin = "6px";
		voFormLot.bottomMargin = "6px";
		voFormLot.leftMargin = "6px";
		voFormLot.horizontalSpacing = "12px";
		voFormLot.verticalSpacing = "12px";
		voFormLot.horizontalSeparatorWidth = 1;
		voFormLot.setColumns(["150px", "1fr"]);
		...
		(function(container) {
			...
			vaColVals.forEach(function(each, idx) {
				...
				var checkSup = RGrid._checkSuppressibleDetailColumn(container, idx, idx, vnBefHeaderCellIndex);
				if(!checkSup.isSuppressible) {
					...
					optDtl.readOnly = false;
					
					if(!ValueUtil.isNull(optDtl.value)) {
						dmParam.setValue(dmParam.getColumnNames()[idx], optDtl.value);
					} else {
						dmParam.setValue(dmParam.getColumnNames()[idx], vcGrid.getRow(voInitValue["rowIdx"]).getValue(dmParam.getColumnNames()[idx]));
					}
				...
				vnBefHeaderCellIndex = checkSup.headerCellIndex;
			});
		})(vcGrpForm);
...
function onBtn1Click3(e) {
	var editable = app.getHostProperty("initValue")["editable"];
	if(!ValueUtil.fixBoolean(editable)) {
		// CUD 전용 팝업이 아닌 경우	
		// 닫기버튼만 있을 때, 수정데이터 전달
