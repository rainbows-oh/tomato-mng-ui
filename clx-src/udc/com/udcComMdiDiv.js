...
function removeAllFloat () {
	...
	if(DIVIDE_OPTION["MINIMIZE"]["SHOW"]) {
		if(DIVIDE_OPTION["MINIMIZE"]["MIN_GROUP"]) {
...
function _sortDialog(psLayout) {
	...
	var voMdiActlRct = vcMdifolder.getActualRect();
	var vnMdiHdrHeight = vcMdifolder.getHeaderControls()[0].getActualRect().height;
...
function onBtnMdiDivClick(e){
	...
	var showConstraint = {
		top: voBtnMdiDivActrlRct.top + voBtnMdiDivActrlRct.height + 6 + "px",
