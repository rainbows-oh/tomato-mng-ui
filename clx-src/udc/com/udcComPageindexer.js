...
function onBodyLoad(/* cpr.events.CEvent */ e){
	...
	voPageInfo.addEventListener("load", function(e){
		var totCnt = voPageInfo.getValue("totalCount");
		var rowSize = voPageInfo.getValue("size");
		var pageIdx = voPageInfo.getValue("page");
		var pageSize = "5";
...
function onPageIndexSelectionChange(/* cpr.events.CSelectionEvent */ e, mPageIndex){
	 ...
	 */
	var voPageInfo = app.getAppProperty("pageInfo");
	voPageInfo.setValue("page", e.newSelection);
...
function onIpbCurrentIdxValueChange(/* cpr.events.CValueChangeEvent */ e){
	 ...
	 */
	var voPageInfo = app.getAppProperty("pageInfo");
	voPageInfo.setValue("page", ipbCurrentIdx.value);
...
function onIpbRecordCountPerPageValueChange(/* cpr.events.CValueChangeEvent */ e){
	 ...
	 */
	var voPageInfo = app.getAppProperty("pageInfo");
	voPageInfo.setValue("page", 1);
	voPageInfo.setValue("size", ipbRecordCountPerPage.value);
...
function onBodyScreenChange(/* cpr.events.CScreenChangeEvent */ e){
	var vbMbScrn = e.screen.name != "EXB-FULL";
	...
	if (vbMbScrn || (vcGrid && vcGrid.getRowCount() == 0)) {
		...
		app.lookup("grpPaging").getLayout().setColumnVisible(0, true);
		app.lookup("grpPaging").getLayout().setColumnVisible(2, true);
	}
}
